...
YOU MUST STRICTLY FOLLOW THE ABOVE FORMATTING RULES. 
YOU ARE NOT ALLOWED TO DEVIATE FROM THEM OR ADD EXTRA EXPLANATIONS.
...
def batched_vision_encoding(model, pixel_values, batch_size=3):
    ...
    NOTE: Qwen3VL has a different architecture than Gemma - it doesn't support
    direct batched encoding through model.visual(). The processor handles all
    image encoding internally, so this function just returns the pixel_values
    unchanged for Qwen.
    
    Args:
        model: Qwen3VLForConditionalGeneration model
        pixel_values: Tensor or nested structure from processor
        batch_size: Number of images to process at once (unused for Qwen)
    
    Returns:
        pixel_values unchanged (Qwen handles batching internally)
    """
    # Qwen3VL processor handles all image encoding - just return as-is
    # The performance gain comes from reducing image resolution via processor settings
    return pixel_values
...
def find_image_blocks(input_ids: torch.Tensor, processor) -> List[Tuple[int, int]]:
    ...
    while i < len(ids):
        if ids[i] == vision_start_token_id:
            start = i + 1
            i += 1
            while i < len(ids) and ids[i] != vision_end_token_id:
                i += 1
            if i < len(ids):
                end = i - 1
                if end >= start:
        ...
        for cell_idx, cell in enumerate(row):
            ...
            if cell_type == "image":
                ...
                if isinstance(cell_content, str) and "TEXT:" in cell_content and "IMAGES:" in cell_content:
                    ...
                    # Handle PIL Image in mixed content - just add placeholder
                    if current_table_string.strip():
                        prompt_content.append({"type": "text", "text": current_table_string})
                        current_table_string = ""
                    prompt_content.append({"type": "image", "image": cell_content})
                ...
                # Handle PIL Image objects in text cells
                if isinstance(cell_content, Image.Image):
                    if current_table_string.strip():
                        prompt_content.append({"type": "text", "text": current_table_string})
                        current_table_string = ""
                    prompt_content.append({"type": "image", "image": cell_content})
...
def process_single_example_with_sparsevlm(
    ...
    """Process a single example with dynamic SparseVLM pruning."""
    ...
    for item in prompt_content:
        if item["type"] == "text":
            ...
            img = item["image"]
            # Handle both file paths (strings) and PIL Image objects
            if isinstance(img, str):
                img = img.replace("\\", "/")
            # Qwen processor accepts both string paths and PIL Images
    ...
    # Count images for batched processing
    ...
    # Extract pixel_values for batched processing if there are many images
    pixel_values = inputs.pop("pixel_values", None)
    
    # Move non-pixel inputs to device
    for k, v in inputs.items():
        if isinstance(v, torch.Tensor):
            inputs[k] = v.to(device)
    
    # Move pixel_values back if present
    # Note: Qwen's architecture doesn't support external batched encoding
    # Performance gains come from reduced image resolution set in processor config
    ...
    try:
        with torch.inference_mode():  # Faster than no_grad
            generation = model.generate(
                **inputs,
                max_new_tokens=542,  # Reduced from 100 for speed
                do_sample=False,
                use_cache=True,
    ...
    # Minimal cleanup
    del inputs, generation, generation_text
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SparseVLM for multimodal table QA with Qwen3-VL")
    parser.add_argument("--debug", action="store_true", help="Debug mode (process only 5 examples)")
    parser.add_argument(
        "--model",
        type=str,
        default=os.getenv("CAPTR_QWEN3VL_MODEL", "Qwen/Qwen3-VL-2B-Instruct"),
        help="Model name (set CAPTR_QWEN3VL_MODEL in the environment or pass an explicit repo id / snapshot path)",
    )
    parser.add_argument(
        "--n_examples",
        type=int,
    ...
    # Compile model for faster inference (~20-30% speedup)
    print("Compiling model with torch.compile (this may take a few minutes on first run)...")
    model = torch.compile(model, mode="reduce-overhead")
    
    processor = AutoProcessor.from_pretrained(model_id, cache_dir=HF_CACHE_DIR, token=HF_TOKEN, trust_remote_code=True)
    
    # CRITICAL PERFORMANCE FIX: Limit image resolution to 256x256
    # Images smaller than this stay the same, larger images are shrunk to this size
    if hasattr(processor, 'image_processor'):
        processor.image_processor.min_pixels = 1  # Allow small images to stay small
        processor.image_processor.max_pixels = 128 * 128  # Maximum 128x128 for speed
        print(f"Image processor settings: max_pixels={processor.image_processor.max_pixels} (128x128)")
    ...
    if args.mmtabreal:
        ...
        for q_type in question_types:
            ...
            if not os.path.exists(dataset_path):
                print(f"Skipping {q_type}: dataset not found at {dataset_path}")
                continue
        ...
        generator = VLLMGeneratorQwen3VL(judge_args, system_prompt=None, gpu_memory_utilization=0.8)
        ...
        judge_generator = VLLMGeneratorQwen3VL(judge_args, system_prompt=None, gpu_memory_utilization=0.8)
