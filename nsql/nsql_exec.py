        ...
        for step in steps:
            ...
            if nsql.startswith("QA("):
                ...
                for sql_item in sql_s:
                    role, sql_item = nsql_role_recognize(
                        sql_item,
                        db.get_header(),
                    ...
                    if role in ["col", "complete_sql"]:
                        ...
                        val = eval(sql_item)
                        sql_executed_sub_tables.append({"header": ["row_id", "val"], "rows": [["0", val]]})
                        ...
                        sql_executed_sub_tables.append(
                            {
                                ...
                                "rows": [
                                    [
                                        "0",
                                        db.get_passage_by_title(sql_item) + db.get_image_caption_by_title(sql_item),
                                    ]
                                ],
                            }
                        )
                ...
                for _sql_executed_sub_table in sql_executed_sub_tables:
                    # Normalize rows to list format before modifying
                    _sql_executed_sub_table["rows"] = normalize_table_rows(_sql_executed_sub_table["rows"])
