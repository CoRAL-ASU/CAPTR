...
from utils.normalizer import convert_df_type, prepare_df_for_neuraldb_from_table, normalize_table_rows
...
class NeuralDB(object):
    def __init__(self, tables: List[Dict[str, Dict]], passages=None, images=None):
        ...
        # in-memory SQLite database didn't work. Hence we use actual file.
        self.tmp_path = "tmp"
    ...
    def add_sub_table(self, sub_table, join="normal", table_name=None, verbose=True):
        ...
        old_table = pd.DataFrame(normalize_table_rows(oring_table["rows"]), columns=oring_table["header"])
        # concat the new column into old table
        sub_table_df_normed = convert_df_type(pd.DataFrame(data=normalize_table_rows(sub_table["rows"]), columns=sub_table["header"]))
        ...
        # ######################
        if join == "normal":
            new_table = old_table.merge(sub_table_df_normed, how="left", on="row_id")  # do left join
        if join == "ordered":
            new_table = pd.merge(sub_table_df_normed, old_table, on="row_id", how="left")
        ...
        if verbose:
            print(
                "Insert column(s) {} (dtypes: {}) into table.\n".format(
                    ", ".join([_ for _ in sub_table["header"]]),  # noqa: C416
