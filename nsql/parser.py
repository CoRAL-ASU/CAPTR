...
from utils.normalizer import normalize_table_rows
...
class TreeNode(object):
    ...
    def rename_father_val(self, val_names):
        if len(val_names) == 1:
            ...
            new_val_equals_str = (
                "'{}'".format(val_name) if isinstance(convert_type(val_name), str) else "{}".format(val_name)
            )
        else:
...
def nsql_role_recognize(nsql_like_str, all_headers, all_passage_titles, all_image_titles):
    ...
    if nsql_like_str in all_headers or nsql_like_str in list(  # noqa: C417
        map(lambda x: x.lower(), all_headers)
    ):
    ...
    if (
        nsql_like_str.lower() in list(map(lambda x: x.lower(), all_passage_titles))  # noqa: C417
    ) and (nsql_like_str.lower() in list(map(lambda x: x.lower(), all_image_titles))):  # noqa: C417
        return "passage_title_and_image_title", orig_nsql_like_str
    else:
        try:
            nsql_like_str_evaled = str(eval(nsql_like_str))
            if (
                nsql_like_str_evaled.lower() in list(map(lambda x: x.lower(), all_passage_titles))  # noqa: C417
            ) and (
                nsql_like_str_evaled.lower() in list(map(lambda x: x.lower(), all_image_titles))  # noqa: C417
            ):
    ...
    if nsql_like_str.lower() in list(map(lambda x: x.lower(), all_passage_titles)):  # noqa: C417
        return "passage_title", orig_nsql_like_str
    else:
        try:
            nsql_like_str_evaled = str(eval(nsql_like_str))
            if nsql_like_str_evaled.lower() in list(  # noqa: C417
                map(lambda x: x.lower(), all_passage_titles)
            ):
    ...
    if nsql_like_str.lower() in list(map(lambda x: x.lower(), all_image_titles)):  # noqa: C417
        return "image_title", orig_nsql_like_str
    else:
        try:
            nsql_like_str_evaled = str(eval(nsql_like_str))
            if nsql_like_str_evaled.lower() in list(  # noqa: C417
                map(lambda x: x.lower(), all_image_titles)
            ):
...
def extract_rows(sub_table):
    if not sub_table or sub_table["header"] is None:
        return []
    answer = []
    rows = normalize_table_rows(sub_table["rows"])
