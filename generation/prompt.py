...
class PromptBuilder(object):
    ...
    def _select_x_prompt(self, df: pd.DataFrame, num_rows: int, few_shot_demonstration=True):
        """
        ...
        elif few_shot_demonstration is True and self.prompt_style in ["create_table_select_3_full_table"]:
            string = "/*\n{} example rows:\nSELECT * FROM w LIMIT {};\n".format(num_rows, num_rows)

        elif few_shot_demonstration is False and self.prompt_style in ["create_table_select_3_full_table"]:
            string = "/*\n"
    ...
    def build_one_shot_prompt(
        ...
        **kwargs,
        ...
        if self.prompt_style == "create_table_select_full_table":
            ...
            one_shot_prompt += self._select_x_prompt(df=table, num_rows=table.shape[0])
        elif self.prompt_style in [
    ...
    def build_generate_prompt(
        ...
        **kwargs,
        ...
        """
        generate_prompt = ""
        passage_context = kwargs.get("passage_context", None)  # for hybridqa
        ...
        if generate_type == ("answer",):
            ...
            if passage_context is not None:
                generate_prompt += "Passage Context:\n" + passage_context + "\n"
        ...
        if self.prompt_style in [
            ...
            if "num_rows" in kwargs.keys():
                ...
                generate_prompt += self._select_x_prompt(df=table, num_rows=num_rows, few_shot_demonstration=False)
            else:
                generate_prompt += self._select_x_prompt(
                    df=table, num_rows=table.shape[0], few_shot_demonstration=False
                )
            ...
            generate_prompt += _create_table_prompt(table, title)
            generate_prompt += self._select_x_prompt(df=table, num_rows=3, few_shot_demonstration=False)
        else:
            raise ValueError("{} is not supported.".format(self.prompt_style))
