...
from utils.wtq.evaluator import check_denotation, to_value_list
...
class Evaluator:
    ...
    def evaluate(self, pred_answer, gold_answer, dataset, allow_semantic=True, question=None):
        if dataset == "wikitq":
            ...
            print(f"⚠️ '{dataset}' is not specified in evaluate() method of evaluator.py. using exact match instead.")
            return self.eval_ex_match(pred_answer, gold_answer, allow_semantic, question)
    ...
    def eval_ex_match(self, pred, gold, allow_semantic=True, question=None):
        ...
        if not allow_semantic:
            ...
            pred = [str_normalize(span) for span in pred]
            gold = [str_normalize(span) for span in gold]
            pred = sorted(list(set(pred)))  # noqa: C414
            gold = sorted(list(set(gold)))  # noqa: C414
            # (1) 0 matches 'no', 1 matches 'yes'; 0 matches 'more', 1 matches 'less', etc.
            if len(pred) == 1 and len(gold) == 1:
                if (pred[0] == "0" and gold[0] == "no") or (pred[0] == "1" and gold[0] == "yes"):
                    return True
