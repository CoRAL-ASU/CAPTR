...
def parse_col(toks, start_idx, tables_with_alias, schema, default_tables=None):
  ...
  """
    ...
    def choose_best_table(default_tables, tok):
        lower_tok = tok.lower()
        candidate = process.extractOne(lower_tok, [table.lower() for table in default_tables])[0]  # noqa: F821
        return candidate
...
def parse_col_unit(toks, start_idx, tables_with_alias, schema, default_tables=None, end_idx=None):
    ...
    if toks[idx] == "distinct":
        idx += 1
        isDistinct = True  # noqa: F841
    agg_id = AGG_OPS.index("none")  # noqa: F841
    idx = parse_col(toks, idx, tables_with_alias, schema, default_tables)
...
def parse_val_unit(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    col_unit1 = None  # noqa: F841
    col_unit2 = None  # noqa: F841
    unit_op = UNIT_OPS.index("none")

    idx = parse_col_unit(toks, idx, tables_with_alias, schema, default_tables)
    if idx < len_ and toks[idx] in UNIT_OPS:
        unit_op = UNIT_OPS.index(toks[idx])  # noqa: F841
        idx += 1
        idx = parse_col_unit(toks, idx, tables_with_alias, schema, default_tables)
...
def parse_condition(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    while idx < len_:
        ...
        not_op = False
        if toks[idx] == "not":
            not_op = True  # noqa: F841
            idx += 1

        assert idx < len_ and toks[idx] in WHERE_OPS, "Error condition: idx: {}, tok: {}".format(idx, toks[idx])
        op_id = WHERE_OPS.index(toks[idx])
        idx += 1
        val1 = val2 = None  # noqa: F841
...
def parse_select(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    while idx < len_ and toks[idx] not in CLAUSE_KEYWORDS:
        ...
        if toks[idx] in AGG_OPS:
            agg_id = AGG_OPS.index(toks[idx])  # noqa: F841
            idx += 1
        idx = parse_val_unit(toks, idx, tables_with_alias, schema, default_tables)
...
def is_valid_schema(schema):
    ...
    for table in schema:
        if "." in table:
            return False
        if any([keyword == table for keyword in CLAUSE_KEYWORDS]):  # noqa: C419
            return False
